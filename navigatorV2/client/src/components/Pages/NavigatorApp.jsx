  ...
  useEffect(() => {
    ...
    if (mockMuseumData.lines) {
      mockMuseumData.lines.filter(l => l.type === 'ext-wall').forEach(line => {
        ...
        ctx.fillStyle = '#f8fafc'; // slightly grey for base floor to make vibrant areas pop
        ctx.fill();
      });
    }

    // Draw Areas Interactively (if defined in JSON)
    if (mockMuseumData.areas) {
      mockMuseumData.areas.forEach(area => {
        ctx.beginPath();
    ...
    if (currentPath.length > 0) {
      ...
      ctx.setLineDash([]);
        ...
        items.forEach((item, index) => {
          ...
          const cy = item.y;
          const label = 'Ex';
          const bgColor = isActive ? '#ef4444' : '#6366f1'; // Red if active, Indigo otherwise

          ctx.save();
          ctx.shadowColor = 'rgba(0,0,0,0.3)';
          ctx.shadowBlur = 8 / mapZoom;
          ctx.fillStyle = bgColor;
          const boxW = (isActive ? 30 : 26) / mapZoom;
          const boxH = (isActive ? 22 : 18) / mapZoom;
          const radius = 8 / mapZoom;
          ...
          if (isActive) {
            ...
            ctx.beginPath();
            ctx.roundRect(cx - (boxW + 8/mapZoom) / 2, cy - (boxH + 8/mapZoom) / 2, boxW + 8/mapZoom, boxH + 8/mapZoom, radius + 2/mapZoom);
          ...
          ctx.fillStyle = 'white';
          ctx.font = `bold ${(isActive ? 10 : 9) / mapZoom}px sans-serif`;
          ctx.textBaseline = 'middle';
          ctx.textAlign = 'center';
          ctx.fillText(label, cx, cy + (1 / mapZoom));
        });
  ...
  const handleCameraScan = async () => {
    try {
      ...
      setTimeout(async () => {
        ...
        const response = await fetch(`/api/QR/${state?.visitId || 'default'}`);
        const data = await response.json();
        alert(`QR Scan result: ${data.item || 'Item found!'}`);
      }, 2000);
    } catch (error) {
