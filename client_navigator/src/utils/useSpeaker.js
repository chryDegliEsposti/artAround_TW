...
export function useSpeaker() {
  ...
  const speak = useCallback(async (text, id = null) => {
    ...
    try {
      const response = await fetch('/api/v1/navigator/ai/request/speak', {
        method: 'POST',
      ...
      // CRITICAL: Check if we are still wanting to speak THIS text after the await
      if (currentTextRef.current !== text || controller.signal.aborted) {
        return;
      }
      ...
      if (data.chunks && data.chunks.length > 0) {
        ...
        if (audio) {
          audio.src = url;
          audio.play().catch(e => {
            // Only log if it's not the interruption error which is expected during stop/pause
