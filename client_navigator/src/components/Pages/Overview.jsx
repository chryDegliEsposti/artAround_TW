    ...
    useEffect(() => {
        const fetchMuseum = async () => {
            ...
            try {
                ...
                const museumResponse = await fetch(`/api/v1/navigator/museums/get/${idToFetch}`);

                if (museumResponse.ok) {
                    const data = await museumResponse.json();
                    if (!data) return; // fallback to existing state or error out
                    ...
                    setMuseumData({
                        ...
                        price: museum.price || "€15.00",
                        hours: museum.hours || "09:00 - 18:00",
                        address: museum.address || "Via dei Musei, 1"
                    });
                }
            } catch (error) {
