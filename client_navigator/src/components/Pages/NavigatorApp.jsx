...
export const fetchMuseumData = async (targetMuseumId = null) => {
  try {
    let url = '/api/v1/navigator/museums/museumData';
    if (targetMuseumId) {
      const isHexId = /^[0-9a-fA-F]{24}$/.test(targetMuseumId);
      url += isHexId ? `?id=${encodeURIComponent(targetMuseumId)}` : `?museumId=${encodeURIComponent(targetMuseumId)}`;
    }
...
export default function NavigatorApp() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const queryMuseumId = searchParams.get('museumId') || searchParams.get('museum') || searchParams.get('id');
        const targetMuseumId = state?.museumId || state?.id || queryMuseumId;
        ...
        if (!visitId) {
          ...
          const fallbackData = await fetchMuseumData(targetMuseumId);
          setMuseumData(fallbackData);
          setVisitData({ 
            title: fallbackData?.name ? `Esplorazione: ${fallbackData.name}` : "Esplorazione Libera", 
            duration: 60,
            museum: fallbackData
          });
    ...
    loadData();
  }, [state?.visitId, state?.museumId, state?.id]);
  ...
  useEffect(() => {
    ...
    const tourItems = (museumData.pois || [])
      ...
      .map(poi => {
        const visitItem = (visitData?.items || []).find(it => 
          (poi.itemRef && ((it._id || it.id)?.toString() === poi.itemRef?.toString())) ||
          (poi.artworkId && it.artworkId === poi.artworkId) ||
          (poi.name && it.title && it.title.trim().toLowerCase() === poi.name.trim().toLowerCase())
        );

        return {
          id: poi.id,
          itemRef: poi.itemRef || visitItem?._id || visitItem?.id,
          name: poi.name || visitItem?.title || 'Opera',
          title: poi.name || visitItem?.title || 'Opera',
          desc: poi.desc || poi.description || visitItem?.description || `Stai navigando verso ${poi.name}.`,
          description: poi.description || poi.desc || visitItem?.description || '',
          image: poi.image || poi.recognitionImage || visitItem?.recognitionImage || '',
          recognitionImage: poi.recognitionImage || poi.image || visitItem?.recognitionImage || '',
          artist: poi.artist || poi.author || visitItem?.author || '',
          author: poi.author || poi.artist || visitItem?.author || '',
          style: poi.style || visitItem?.style || '',
          museumId: museumData.museumId,
          x: tx(poi.position.x),
          y: ty(poi.position.y),
          layerId: poi.layerId,
          artworkId: poi.artworkId || visitItem?.artworkId
        };
      });
    setItems(tourItems);
  }, [museumData, visitData, tx, ty]);
        ...
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '1.5rem', padding: '2.5rem 2rem', maxWidth: '460px', width: '100%', textAlign: 'center', color: '#f8fafc', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.5)' }}>
          ...
          {visitData && (
            <div style={{ background: '#0f172a', borderRadius: '1rem', padding: '1rem', marginBottom: '1.5rem', textAlign: 'left', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div style={{ fontSize: '0.8rem', color: '#64748b', marginTop: '0.2rem' }}>{visitData?.museum?.name || museumData?.name || "Museo"}</div>
  ...
  return (
    <div className="navigator-container">
      ...
      <QuizPickerModal
        ...
        museumId={museumData?.museumId || visitData?.museum?.museumId || 'PIN-BO'}
        museumName={museumData?.name || visitData?.museum?.name || 'Museo'}
